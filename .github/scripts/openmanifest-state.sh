...
usage() {
  echo "usage: openmanifest-state.sh read <state-key> <output> | write <state-key> <input>" >&2
  exit 2
}
...
if [[ ! "$state_key" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]]; then
  echo "invalid OpenManifest state key: $state_key" >&2
  exit 2
fi

state_branch="${OPENMANIFEST_STATE_BRANCH:-${STACKPORT_STATE_BRANCH:-openmanifest-state}}"
if [[ ! "$state_branch" =~ ^[A-Za-z0-9][A-Za-z0-9._/-]*$ ]] \
  || [[ "$state_branch" == *".."* ]] \
  || [[ "$state_branch" == *"//"* ]] \
  || ! git check-ref-format "refs/heads/$state_branch" >/dev/null 2>&1; then
  echo "invalid OpenManifest state branch: $state_branch" >&2
  exit 2
fi
...
fetch_state_branch() {
  ...
  case "$remote_status" in
    ...
    2) return 1 ;;
    *)
      echo "Unable to inspect remote OpenManifest state branch '$state_branch'." >&2
      return 3
      ;;
  esac
  ...
  if ! git -C "$repository" fetch --quiet --no-tags origin \
    ...
    echo "Unable to fetch remote OpenManifest state branch '$state_branch'." >&2
    return 3
  fi
}
...
read_state() {
  ...
  if fetch_state_branch; then
    ...
    if [[ "$fetch_status" -eq 1 ]]; then
      echo "No remote OpenManifest state branch found; planning from empty state."
      return 0
    fi
    return "$fetch_status"
  fi
  ...
  echo "Loaded OpenManifest state '$state_key' from '$state_branch'."
}
...
if state.get("schema_version") not in {"openmanifest-state/v1", "stackport-state/v1"}:
    raise SystemExit("state schema_version must be openmanifest-state/v1")
...
write_state() {
  [[ -s "$state_file" ]] || {
    echo "OpenManifest state file is missing or empty: $state_file" >&2
    exit 2
  }
  validate_state

  worktree="$(mktemp -d "${RUNNER_TEMP:-/tmp}/openmanifest-state.XXXXXX")"
  cleanup() {
  ...
  if fetch_state_branch; then
    ...
    printf '%s\n' \
      '# OpenManifest State' \
      '' \
      'This branch is managed by OpenManifest CI. State files contain provider identifiers and secret references, not secret values.' \
      >"$worktree/README.md"
  fi
  ...
  if git -C "$worktree" diff --cached --quiet; then
    echo "OpenManifest state '$state_key' is unchanged."
    return 0
  fi

  git -C "$worktree" config user.name "openmanifest-ci"
  git -C "$worktree" config user.email "openmanifest-ci@users.noreply.github.com"
  git -C "$worktree" commit --quiet -m "Update OpenManifest state: $state_key"
  git -C "$worktree" push --quiet origin "HEAD:refs/heads/$state_branch"
  echo "Published OpenManifest state '$state_key' to '$state_branch'."
}
