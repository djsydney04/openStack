...
try:
    ...
    fail(f"OPENMANIFEST_SECRETS_JSON is not valid JSON: {error}")

if not isinstance(secrets, dict):
    fail("OPENMANIFEST_SECRETS_JSON must be a JSON object")

for name, value in secrets.items():
    if not isinstance(name, str) or not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", name):
        fail(f"invalid environment variable name in OPENMANIFEST_SECRETS_JSON: {name!r}")
...
with open(environment_file, "a", encoding="utf-8") as handle:
    for name, value in secrets.items():
        ...
        delimiter = f"OPENMANIFEST_{uuid.uuid4().hex}"
