...
temporary="$(mktemp -d "${TMPDIR:-/tmp}/openmanifest-ci-test.XXXXXX")"
cleanup() {
  rm -rf "$temporary"
}
trap cleanup EXIT
...
git -C "$repository" config user.name "OpenManifest Test"
git -C "$repository" config user.email "openmanifest-test@example.com"
...
GITHUB_WORKSPACE="$repository" \
  "$root/.github/scripts/openmanifest-state.sh" read production "$empty_state"
...
cat >"$state" <<'JSON'
{
  "schema_version": "openmanifest-state/v1",
  "provider": "vercel",
  "providers": [],
  "resources": []
}
...
GITHUB_WORKSPACE="$repository" \
  "$root/.github/scripts/openmanifest-state.sh" write production "$state"
loaded="$temporary/loaded.json"
GITHUB_WORKSPACE="$repository" \
  "$root/.github/scripts/openmanifest-state.sh" read production "$loaded"
cmp "$state" "$loaded"

legacy_state="$temporary/legacy-state.json"
cp "$state" "$legacy_state"
...
state["schema_version"] = "stackport-state/v1"
path.write_text(json.dumps(state, indent=2) + "\n")
PY
GITHUB_WORKSPACE="$repository" \
  "$root/.github/scripts/stackport-state.sh" write legacy "$legacy_state"

git -C "$repository" remote set-url origin "$temporary/missing.git"
if GITHUB_WORKSPACE="$repository" \
  "$root/.github/scripts/openmanifest-state.sh" read production "$loaded"; then
...
if GITHUB_WORKSPACE="$repository" \
  "$root/.github/scripts/openmanifest-state.sh" write '../unsafe' "$state"; then
...
if GITHUB_WORKSPACE="$repository" \
  "$root/.github/scripts/openmanifest-state.sh" write invalid "$invalid_state"; then
...
OPENMANIFEST_SECRETS_JSON='{"VERCEL_TOKEN":"token-value","MULTILINE":"line one\nline two"}' \
  GITHUB_ENV="$github_env" \
  python3 "$root/.github/scripts/export_openmanifest_secrets.py" >"$mask_output"
grep -q '^VERCEL_TOKEN<<OPENMANIFEST_' "$github_env"
grep -q '^MULTILINE<<OPENMANIFEST_' "$github_env"
grep -q '^::add-mask::token-value$' "$mask_output"

legacy_github_env="$temporary/legacy-github-env"
STACKPORT_SECRETS_JSON='{"LEGACY_TOKEN":"legacy-value"}' \
  GITHUB_ENV="$legacy_github_env" \
  python3 "$root/.github/scripts/export_stackport_secrets.py" >/dev/null
grep -q '^LEGACY_TOKEN<<OPENMANIFEST_' "$legacy_github_env"

if OPENMANIFEST_SECRETS_JSON='["not-an-object"]' GITHUB_ENV="$github_env" \
  python3 "$root/.github/scripts/export_openmanifest_secrets.py"; then
...
echo "OpenManifest CI helper integration test passed."
