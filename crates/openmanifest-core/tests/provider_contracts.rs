...
use openmanifest_core::providers::{provider_registry, ProviderOperation};
...
const EXAMPLES: [(&str, &str); 4] = [
    ("vercel", "vercel.openmanifest.yaml"),
    ("supabase", "supabase.openmanifest.yaml"),
    ("neon", "neon.openmanifest.yaml"),
    ("railway", "railway.openmanifest.yaml"),
];
...
fn provider_access_probes_are_read_only_and_return_sanitized_status() {
    ...
    assert!(plan.requests.iter().all(|request| {
        request.operation == ProviderOperation::Read
            && matches!(
                request.method,
                openmanifest_core::providers::HttpMethod::Get
                    | openmanifest_core::providers::HttpMethod::Post
            )
    }));
...
fn vercel_team_scope_is_encoded_and_composes_with_upsert() {
    let mut spec = read_example("vercel.openmanifest.yaml");
    spec.targets
        .get_mut("production")
    ...
    let server = thread::spawn(move || {
        ...
        for (request_line, response_body) in expected {
            ...
            if request_line.starts_with("POST") {
                assert!(request.contains("OpenManifestProviderProbe"), "{request}");
            }
            let response = format!(
