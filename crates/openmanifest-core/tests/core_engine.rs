use openmanifest_core::analysis::analyze_portability;
use openmanifest_core::apply::{apply_plan, DryRunAdapter};
use openmanifest_core::diff::{diff_state, DiffAction};
use openmanifest_core::importers::{
    import_supabase, import_vercel, SupabaseProject, VercelEnvironmentVariable, VercelProject,
};
use openmanifest_core::manifest::{
...
use openmanifest_core::planner::{create_plan, create_plan_with_state, PlanAction};
...
use openmanifest_core::state::{StackState, StateResource};
...
#[test]
fn accepts_legacy_manifest_versions_during_rebrand() {
    let mut manifest = sample_manifest();
    manifest.schema_version = "stackport/v1".to_string();
    assert!(validate_manifest(&manifest).unwrap().valid);

    let mut spec: StackSpec =
        serde_yaml::from_str(include_str!("../../../fixtures/app.openmanifest.yaml")).unwrap();
    spec.version = "stackport/app/v1alpha1".to_string();
    assert!(
        validate_stack_spec(&spec, Some("production"))
            .unwrap()
            .valid
    );
}
...
fn diffs_manifest_against_state() {
    ...
    let state = StackState {
        schema_version: "openmanifest-state/v1".to_string(),
...
fn handles_json_rpc_version_and_plan() {
    let version_response = handle_rpc_request(JsonRpcRequest {
        ...
        method: "openmanifest.version".to_string(),
        params: serde_json::json!({}),
    });
    assert!(version_response.error.is_none());
    assert_eq!(
        version_response.result.unwrap()["rpc_version"],
        serde_json::json!("2026-08-14")
    );

    let legacy_version_response = handle_rpc_request(JsonRpcRequest {
        jsonrpc: "2.0".to_string(),
        id: serde_json::json!("legacy"),
        method: "stackport.version".to_string(),
        params: serde_json::json!({}),
    });
    assert!(legacy_version_response.error.is_none());
    assert_eq!(
        legacy_version_response.result.unwrap()["rpc_version"],
        serde_json::json!("2026-07-23")
    );

    let app_manifest_response = handle_rpc_request(JsonRpcRequest {
        jsonrpc: "2.0".to_string(),
        id: serde_json::json!("manifest"),
        method: "appManifest.validate".to_string(),
        params: serde_json::json!({
            "spec": serde_yaml::from_str::<serde_json::Value>(include_str!(
                "../../../fixtures/app.openmanifest.yaml"
            ))
            .unwrap(),
            "target": "production"
        }),
    });
    assert!(app_manifest_response.error.is_none());
...
fn converts_yaml_stack_spec_into_manifest_for_target_provider() {
    let spec: StackSpec =
        serde_yaml::from_str(include_str!("../../../fixtures/app.openmanifest.yaml"))
...
fn builds_read_and_import_plans_from_provider_state() {
    ...
    let state = StackState {
        schema_version: openmanifest_core::STATE_SCHEMA_VERSION.to_string(),
...
fn builds_vercel_and_supabase_create_requests() {
    ...
    supabase_manifest.variables.insert(
        ...
        openmanifest_core::manifest::Variable {
            description: None,
...
fn resolves_credentials_and_secrets_only_inside_transport_boundary() {
    let request_plan = ProviderRequestPlan {
        requests: vec![ProviderRequest {
            ...
            method: openmanifest_core::providers::HttpMethod::Post,
            url: "https://provider.invalid/secrets".to_string(),
            operation: openmanifest_core::providers::ProviderOperation::ApplyCreate,
...
fn supabase_edge_function_uses_real_multipart_upload() {
    let source_path = std::env::temp_dir().join(format!(
        "openmanifest-edge-function-{}.ts",
        std::process::id()
    ));
    std::fs::write(
        &source_path,
        "Deno.serve(() => new Response('openmanifest'));",
    )
    ...
    let server = thread::spawn(move || {
        ...
        assert!(request.contains("openmanifest-edge-function-"));
        assert!(request.contains("Deno.serve(() => new Response('openmanifest'));"));
...
fn action_for(plan: &openmanifest_core::planner::MigrationPlan, resource_id: &str) -> PlanAction {
    plan.steps
        .iter()
